...
> **Reconciliation 2026-06-04 (decisions-of-record live in `orchestrator-visual-language.md` §3/§6).** This list is the exploratory archive; most entries below are no longer open. **RESOLVED/RATIFIED of record:** OQ-1.1, OQ-1.2, OQ-1.3, OQ-2.1 (→ Option A, no Player-Output snapshot — owner decision 2026-06-04), OQ-2.2, OQ-3.3, OQ-4.1, OQ-5.1, OQ-6.2/D6.2 (camera relabel shipped, phase-18), OQ-7.1, OQ-7.2, OQ-8.1, OQ-8.2, OQ-8.3, OQ-9.1, OQ-9.2 — see the §3 decisions table for each call/confidence/tag. **Genuinely still open (taste/policy):** OQ-4.2 (mobile pending-edits), OQ-6.3 (390px proximity), OQ-9.3 (folder-action pointer scope, overlaps OQ-5.2). **e2e-blocked verification residue:** OQ-8.1's composited `color-mix()` contrast. The code-verified status of the few remaining *build* items (D1.1 no-hero guard, D1.2/D1.3 `/` accelerator, etc.) is recorded in `docs/analysis/orchestrator_hig_ux_backlog_2026_05_30.md` → "Residual research items".
