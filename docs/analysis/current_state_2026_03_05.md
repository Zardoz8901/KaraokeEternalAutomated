...
|------|--------|-------|-----------|-----|
| 2026-06-03 | Orchestrator camera source-binding relabel: Stage cameraPipeline now says `Source bound`, `Source binding partial`, or `Source: no camera`, exposes that binding status through a polite status region, and the ownership guard covers the binding surface while preserving the separate `Camera live` connection pill | Standard | `hydraPreviewUtils.ts`, `StagePanel.tsx`, `orchestratorStatusOwnership.test.ts` | `docs/analysis/orchestrator_camera_binding_relabel_2026_06_03.md` |
| 2026-06-02 | Orchestrator typography ramp: added semantic size aliases for meta/body/control/title, raised all audited sub-0.75rem Orchestrator CSS font sizes to the floor, promoted Stage title to the title rung, and added a font-size floor audit | Standard | `OrchestratorView.css`, `StagePanel.css`, `ApiReference.css`, `PresetPicker.css`, `orchestratorColorAudit.test.ts` | `docs/analysis/orchestrator_typography_ramp_2026_06_02.md` |
| 2026-06-02 | Orchestrator Local Preview fit-window sizing: StagePanel now measures the actual Stage frame with ResizeObserver, coalesces preview resize updates, preserves getPreviewSize as fallback, and adds camera-binding regression coverage for width/height-only Hydra resizes | High-Risk | `orchestratorLayout.ts`, `StagePanel.tsx`, `StagePanel.css`, preview/visualizer tests | `docs/analysis/orchestrator_preview_fit_window_2026_06_02.md` |
| 2026-06-02 | Orchestrator host spatial conformance: hostSplit Stage now owns the desktop growth row while Code is bounded, and a static/render guard locks the pre-Option-B Stage frame as single-preview/flex-only with no Player Output snapshot cell | Standard | `OrchestratorView.css`, `OrchestratorView.spatial.test.tsx` | `docs/analysis/orchestrator_spatial_host_row_ratio_2026_06_02.md` |
| 2026-06-01 | Orchestrator role-token layer: landed loaded/selected/synced/reference roles, shared tone/motion/z scales, routed scoped state/badge/status CSS through role tokens, added the send-ack confirmation beat, and extended the color audit for role recipes, contrast guards, z migration, and reduced-motion coverage | Standard | `OrchestratorView.css`, Orchestrator component CSS, `orchestratorColorAudit.test.ts` | `docs/analysis/orchestrator_role_token_layer_2026_06_01.md` |
| 2026-05-31 | Orchestrator operator feedback + camera ownership: preset-row Send now shows glyph-only row-local pending/success/failure feedback with a mobile Presets-tab dot, while the Stage strip keeps broadcast text ownership; camera source binding is relabeled as `Source ...` and included in the ownership audit | Standard | `useOrchestratorWorkspace.ts`, `PresetTree.tsx`, `OrchestratorView.tsx`, `StagePanel.tsx`, ownership tests | `docs/analysis/orchestrator_operator_feedback_camera_ownership_2026_05_31.md` |
| 2026-05-31 | Orchestrator PresetTree row stability: row click/Enter now selects only, explicit Load owns local preview loading, badges render/announce strongest-truth-first, Gallery identity appears on rows, and the badge lane is fixed-height/no-wrap to avoid mid-set reflow | Standard | `PresetTree.tsx`, `PresetTree.css`, `PresetBrowser.tsx`, tests | `docs/analysis/orchestrator_preset_row_stability_2026_05_31.md` |
| 2026-05-31 | Orchestrator semantic color role map: added `--orch-success` and `--orch-applied`, retuned Applied-on-Player to violet, Cam to cyan, synced to green, and locked live/applied/success as distinct Solarized role hues in the color audit | Standard | `OrchestratorView.css`, `PresetTree.css`, `OrchestratorStatusStrip.css`, `orchestratorColorAudit.test.ts` | `docs/analysis/orchestrator_color_role_map_2026_05_31.md` |
| 2026-05-31 | Early-HiG red-team of the two shape decisions (dense-row presets, header-strip status): verdict keep-with-mitigations for both — sound under early Macintosh HiG; surviving debts are perceived-stability (reflowing rows → reserved fixed-height badge lane) and feedback-and-dialog (Operator row-Send → row-local non-textual ack, strip keeps the text). Resolves OQ-3.1/3.2/6.1; executes FIRM D3.2/D6.2; on-frame chips stay rejected | Docs | `docs/architecture/orchestrator-style-directions.md` (Decisions section) | `docs/analysis/orchestrator_shape_redteam_early_hig_2026_05_31.md` |
| 2026-05-31 | Orchestrator Style Directions (workflow): expanded the 9 visual-system principles into concrete per-principle style directions tagged DONE/PARTIAL/OPEN against HEAD, guideline-cited (Apple HiG/WCAG SC/NN-g/M3), with 23 consolidated open questions; adversarially reviewed (re-audited so shipped Gate-3 a11y/token work is marked landed; WCAG citations corrected; elevation/contrast reconciled; drag-lift shadow exception, FORBIDDEN substring + camera same-slice notes added) | Docs | `docs/architecture/orchestrator-style-directions.md`, `orchestrator-visual-system.md`, `ARCHITECTURE.md` | (workflow `wf_2ac82513-3d9`) |
| 2026-05-30 | Gate 3e/3f Stage header layout: Stage header order locked to left label/Preview qualifier, center truncating status strip, and right camera/buffer controls; mobile keeps the Preview qualifier visible and preserves 44px buffer targets | Standard | `StagePanel.css`, `OrchestratorStatusStrip.css`, `StagePanel.test.tsx` | `docs/analysis/orchestrator_stage_header_layout_2026_05_30.md` |
